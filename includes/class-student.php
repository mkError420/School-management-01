...
class Student {
	...
	public static function add( $student_data ) { 
		...
		if ( empty( $student_data['first_name'] ) || empty( $student_data['roll_number'] ) || empty( $student_data['dob'] ) || empty( $student_data['gender'] ) || empty( $student_data['parent_name'] ) || empty( $student_data['parent_phone'] ) || empty( $student_data['address'] ) ) {
			return new \WP_Error( 'missing_fields', __( 'Please fill all the required fields.', 'school-management-system' ) );
		}
